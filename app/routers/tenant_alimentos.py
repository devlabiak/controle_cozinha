...
@router.delete("/{tenant_id}/limpar-todos-produtos")
async def limpar_todos_produtos(
    ...
    Para confirmar, passe o parâmetro: ?confirmar=SIM_DELETAR_TUDO
    """
    ...
    if confirmar != "SIM_DELETAR_TUDO":
        return {
            "aviso": "⚠️ ATENÇÃO: Esta operação vai DELETAR PERMANENTEMENTE todos os dados!",
            "instrucoes": "Para confirmar, adicione o parâmetro: ?confirmar=SIM_DELETAR_TUDO",
            "exemplo": f"/api/tenant/{tenant_id}/limpar-todos-produtos?confirmar=SIM_DELETAR_TUDO",
            "o_que_sera_deletado": [
                "Todas as movimentações de estoque (entradas e saídas)",
                "Todos os lotes de produtos",
                "Todos os produtos/alimentos cadastrados"
            ]
        }
